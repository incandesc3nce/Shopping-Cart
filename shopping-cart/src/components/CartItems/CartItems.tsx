    ...
    <section>
      ...
      <div className="m-2 flex flex-col gap-4">
        {array.map((arrayItem: any) => (
          <article className="min-h-30 flex items-center justify-between rounded-3xl bg-slate-200 p-1 shadow-md">
            <img
              src={arrayItem.item.imgUrl}
              alt={arrayItem.item.title}
              className="h-20 w-20 rounded-full object-cover"
            />
            <h3 className="max-w-24 text-center text-xl font-semibold">
        ...
        <p>Total: ${array.reduce((acc: any, item: any) => acc + item.item.price * item.quantity, 0)}</p>
      </div>
    </section>
  );
};
